ifeq (0,1)
Command to run UVM:
step0 : make -f uvm_singleadd.mk dpi
Step1 : make -f uvm_singleadd.mk build
Step2 : make -f uvm_singleadd.mk <test_case> (replace test_case in following test_case list, no need <> when running)
...
(If there are some Make error, try: make -f uvm_singleadd.mk clean)
endif

# Makefile
TB = uvm_tb/uvm_tb_add_single/tb_adder.sv
TOP = $(TB)
AGENT = $(TB)/fadd_agent
RTL = src/single_precision/adder.sv
...
build: 
	vlog +incdir+src/single_precision \
	+incdir+include \
	+incdir+$(AGENT) \
	+acc \
	+cover \
...
run: build
	vsim -c -sv_lib $(DPI_LIB) tb_adder -L \
	$$QUESTA_HOME/uvm-1.2 \
	-voptargs=+acc \
	-coverage \
